...
int Testbench::write_bmp(string outfile_name) {
  ...
  // file size
  file_size = width * height * bytes_per_pixel + output_rgb_raw_data_offset;  header[2] = (unsigned char)(file_size & 0x000000ff);
...
void Testbench::do_gaussian() {
  ...
  cout << "# of total pixels transfered from Testbench to GauFilter:" << total_pixels << endl;
  sc_stop();
}
